...
#如果文件夹不存在，创建文件夹
if [ ! -d "$AIMS_DIR/storage/data/M00" ]; then
  mkdir $AIMS_DIR/storage/data/M00
fi
#软连接
ln -s $AIMS_DIR/storage/data  $AIMS_DIR/storage/data/M00
#创建软连接后关闭
# /etc/init.d/fdfs_storaged stop
#启动nginx
cp $ORIGIN_DIR/config/nginx_storage.conf /home/husen/nginx/conf/nginx.conf
nginx
echo "-------------  nginx 启动成功"
echo "-------------  安装完成  -------------------"