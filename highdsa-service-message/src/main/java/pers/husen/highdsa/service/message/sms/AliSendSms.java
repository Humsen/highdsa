...
import pers.husen.highdsa.common.exception.json.JacksonException;
...
public class AliSendSms implements SendSms {
	...
	public SmsSendResponse sendSmsCaptcha(String phoneNumber, String chptcha) throws UnsupportedEncodingException, ClientException, IOException {
		...
		smsSendParams.setSignName(MessageConstants.SIGN_NAME);
		// 现在写死了,以后要动态设置为templateId
		smsSendParams.setTemplateCode(MessageConstants.TEMPLATE_REGISTER);
		...
		logger.debug("发送成功, {}", mapper.writeValueAsString(request));
	...
	public SmsSendResponse sendSmsNotice(String phoneNumber, String userName, String chptcha) throws UnsupportedEncodingException, ClientException, IOException {
		...
		smsSendParams.setTemplateCode(MessageConstants.TEMPLATE_ADVICE);
		smsSendParams.setPhoneNumbers(phoneNumber);
		smsSendParams.setTemplateParam(templateParam);
	...
	public Map<?, ?> querySendDetailsByBizId(String phoneNumber, String bizId) throws UnsupportedEncodingException, ClientException, IOException {
		...
		logger.debug("查询成功, {}", mapper.writeValueAsString(querySendDetailsRequest));

		return response;
	}

	@Override
	public Map<?, ?> querySendDetails(SmsQueryRequest paramMap) throws UnsupportedEncodingException, ClientException, IOException {
		...
		logger.debug("查询成功, {}", mapper.writeValueAsString(smsQueryRequest));
