 ...
 * @Author 何明胜
 *
 * @Created at 2018年5月14日 下午5:53:35
 * 
 * @Version 1.0.3
 */
public class CustUserEmailRealm extends AuthorizingRealm {
	private static final Logger logger = LogManager.getLogger(CustUserEmailRealm.class.getName());
	...
	public boolean supports(AuthenticationToken token) {
		...
		return LoginType.EMAIL == custToken.getLoginType();
	}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		CustUser userRole = custUserManager.findRolesByUserEmail(userName);
		if (userRole == null) {
			return null;
		}
		...
		CustUser userPermission = custUserManager.findPermissionsByUserEmail(userName);
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		CustUser user = custUserManager.findUserByUserEmail(username);
	...
	public void clearCachedAuthorizationInfo(String userEmail) {
		if (BaseUtils.isEmpty(userEmail)) {
			return;
		}
	...
	public void clearCachedAuthenticationInfo(String userEmail) {
		if (BaseUtils.isNotEmpty(userEmail)) {
	...
	public void clearAllCache(String userEmail) {
		this.clearCachedAuthenticationInfo(userEmail);
		this.clearCachedAuthorizationInfo(userEmail);
	}
